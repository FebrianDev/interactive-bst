export default function (listLog) {
    ...
    ${importLib}    
        ...
        struct node *search(struct node *root, float x) {
        ...
        struct node *insert(struct node *root, float x) {
            if (root == NULL)
                return new_node(x);
            else if (x > root->data)
        ...
        struct node *removeData(struct node *root, float x) {
            if (root == NULL)
                return NULL;
            if (x > root->data)
   ...
   `
    ...
    const newList = []
    listLog.map((data, keyValue) => {
        const key = data.split("(")[0]
        const value = data.split("(")[1].split(")")[0]
        if (key === "insert" && keyValue > 0) newList.push("insert(root" + "! " + value + ")")
